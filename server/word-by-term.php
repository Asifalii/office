...
/*$term_sql = "SELECT id,word FROM `wordbyterm`";

$term_query = mysql_query($term_sql) or die(mysql_error());

while ($row = mysql_fetch_assoc($term_query)) {
   //print $row['word'];
   
 
   $mean_query = mysql_query('select mean from x_'.$lang.' where word = "'.$row['word'].'"');
   
   $mean = mysql_fetch_assoc($mean_query);
   
   if(isset($mean['mean']) && $mean['mean'] != null){
	  
	   $udpate_term = mysql_query('UPDATE wordbyterm SET meaning_found="1" WHERE id = '.$row['id']);
   }
}*/
...
$sql = "select DISTINCT cat from wordbyterm";

$cat_query = mysql_query($sql) or die(mysql_error());

while ($row = mysql_fetch_assoc($cat_query)) {
    $cats[] = $row['cat'];
}
...
$PageSql = "SELECT * FROM `wordbyterm` where `cat` = '".$term_cat."' AND `meaning_found` = '1'";
...
$terms_sql = "select * from wordbyterm where `cat` = '".$term_cat."' AND `meaning_found` = '1' LIMIT ".$start.", ".$perpage;

$terms_query = mysql_query($terms_sql) or die(mysql_error());
...
<div class="container">
					...
					<div class="jumbotron">
                         ...
                         <p id="prev_date">
							<?php foreach($cats as $cat) { ?>
								<a class="btn btn-primary btn-raised" href="<?php echo $url.'?cat='.$cat;?>" title="<?php echo ucfirst($cat)?>"><?php echo ucfirst($cat)?></a>
							<?php } ?>
						 </p>
						 
						 <div class="panel">
							<div class="panel-heading"><?php echo ucfirst($term_cat); ?></div>
							...
							$words = array();
							$word_link_array = array();
							while ($row = mysql_fetch_assoc($terms_query)) {
								$words[] = strtolower($row['word']);
								$word_link_array[strtolower($row['word'])] = $row['link'];
							}
							$mean_query = mysql_query('select mean,word from x_'.$lang.' where word in ("'.implode('","', $words).'") limit '.count($words));
							$word_mean_array = array();
							while($mean_row=mysql_fetch_assoc($mean_query)){
								
								$word_mean_array[$mean_row['word']] = $mean_row['mean'];
							}
							
							foreach($words as $key=>$val){
								
							?>
									<p><img src="<?php echo $url.'/img/term/'.$term_cat.'/'.$word_link_array[$val]; ?>" alt="<?php echo ucfirst($word_link_array[$val]);?>" width="200"/>
									<span><?php echo $word_mean_array[$val];?></span></p>
							<?php }?>
							
							<ul class="pagination">
								<?php if($curpage != $startpage){ ?>
    <li class="page-item">
      <a class="page-link" href="?page=<?php echo $startpage.'&cat='.$term_cat ?>" tabindex="-1" aria-label="Previous">
    ...
    <li class="page-item">
      <a class="page-link" href="?page=<?php echo $endpage.'&cat='.$term_cat ?>" aria-label="Next">
	...
	<li class="page-item active"><a class="page-link" href="?page=<?php echo $curpage ?>"><?php echo $curpage ?></a></li>
							...
							<?php if($curpage != $endpage){ ?>
    <li class="page-item"><a class="page-link" href="?page=<?php echo $nextpage.'&cat='.$term_cat ?>"><?php echo $nextpage ?></a></li>
<?php } ?>
							</ul>
