...
Also Available Different ' . ucfirst($lang) . ' keyboard layout To Typing ' . ucfirst($lang) . ' To English Translate And English To ' . ucfirst($lang) . ' Converter. ' . "Let's Enjoy It...";
    ...
    <meta property="og:url" content="<?= $base_url. $lang .'-to-english-dictionary' ?>"/>
    ...
    if (in_array($lang, array_keys($k_array)) && isset($_GET['keyboard_status']) && !$_GET['keyboard_status']) {
        ...
        <script src="<?= $contentUrl ?>keyboard/keyboard.I.js" type="text/javascript"></script>
...
<div id="site_wrapper">
            ...
            <div class="logo_div">
                ...
                <script>
                    ...
                    function submit_search_local(val) {
						...
						t = [];
						var word = val;
						null != localStorage.getItem("search_local_history") && (t = JSON.parse(localStorage.getItem("search_local_history"))), t.indexOf(word) !== -1 && t.splice(t.indexOf(word), 1), t.push(word), localStorage.setItem("search_local_history", JSON.stringify(t.filter(onlyUnique)));
									
                        window.location.replace(
                            '<?=$base_url?><?=$lang?>-to-english-meaning?word=' + replaced,
                            '_self'
                        );
					...
					var e = JSON.parse(localStorage.getItem("search_local_history"));
				...
				function redirectLocalUrl() {
					...
					null != localStorage.getItem("search_local_history") && (t = JSON.parse(localStorage.getItem("search_local_history"))), t.indexOf(word) !== -1 && t.splice(t.indexOf(word), 1), t.push(word), localStorage.setItem("search_local_history", JSON.stringify(t.filter(onlyUnique)));
					var redirect_url = main_domain() + lang1 + "-to-english-meaning?word=" + word;
                ...
                <div class="search_fld" style="width: 100%;">
                    <form autocomplete="off" name="new_form" action="<?= $base_url . $lang.'-to-english-meaning'; ?>" id="new_form">
    ...
    <div class="content_wrapper top_margin">
        <div class="left_content">
			...
			<div class="box_wrapper">
                    <fieldset>
					<div style="display:none" id="local_lang"><?= $lang ?></div>
					<legend><h1><?= ucfirst($lang) ?> to English Meaning:  <span class="show_word"></span></h1></legend>
						<div class="fieldset_body inner_details" id="local_meaning">
            ...
            <div class="box_wrapper">
				...
				<?php foreach(range('a','z') as $row){ ?>
					<button class="btn_default4 bdr_radius2" onclick="location.href='<?= $base_url.'english-to-'. $lang.'-dictionary-letter-'.strtolower($row)?>'"><?= ucfirst($row)?></button>
				<?php }
				echo '<br>';
				echo '<br>';
				...
				if ($other === false) {
				 ...
				 <button class="btn_default4 bdr_radius2" onclick="location.href='<?= $base_url. $lang . '-to-english-dictionary-letter-'.$value?>'"><?= $value?></button>
			<?php }
					}
				?>
				</fieldset>
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (obj['main']) {
                        ...
                        $mean += '<span><div class="align_text  custom_font2">' + $q + ' : </div><div class="align_text2">' + obj['main'] + '</div><label class="img_align"> Pronunciation:<button class="icon_button" onclick="textToSpeech(\'' + $q + '\')"><img src="https://content2.mcqstudy.com/bw-static-files/stage/img/microphone1.png" loading="lazy"/></button></label><label class="img_align">Add to Favorite: <button class="icon_button" onclick="calFavs(\'' + $q + '\', 1)"><img src="https://content2.mcqstudy.com/bw-static-files/stage/img/heart-add.png" loading="lazy"/></button></label></span>'
	...
	<script>
$("#local_meaning").html('<div class="loader"><img src="https://content2.mcqstudy.com/bw-static-files/imgs/loader.gif"/></div>')
...
$("#local_title").html(lang[0].toUpperCase() + lang.slice(1) + ' to English meaning of ' + word + ' - '+ base_url.replace('http://', ''));
if(word){
	$.ajax({
                type: "get",
                url: "https://server2.mcqstudy.com/get_local_meaning.php",
                data: {
                    word: word,
                    lang: lang,
                },
					...
					var mean = '<div>';
						...
						if (result != '' && result != null) {
                            $.each(result, function (i, val) {
								var res = val.split(" :: ");
                              mean += '<span><a href="'+ base_url + '/english-to-' + lang +'-meaning-'+res[0] +'">' + val + '</a></span>';
                            });
							mean += '</div>';
						
						$("#local_meaning").html(mean);
                        }else{
							$("#local_meaning").html('No word meaning found for '+ word + ' in ' + lang[0].toUpperCase() + lang.slice(1)).css({"font-weight":"bold","color":"#129787"});
						}
	...
	$("#local_meaning").html('No word found');
}
