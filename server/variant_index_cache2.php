...
if ($_GET['q'] && $_GET['lang']) {
    ...
    $main_loop_query = mysqli_query($conn, 'SELECT variant,word FROM variants where variant = "' . $_GET['q'] . '" limit 1');
} else {
    $main_loop_query = mysqli_query($conn, 'SELECT variant,word FROM variants where word != variant');
}
...
while ($main_loop_row = mysqli_fetch_assoc($main_loop_query)) {
    ...
    $query = "SELECT * FROM wordnet WHERE `word` = '" . $main_loop_row['variant'] . "'";
    $result = mysqli_query($conn, $query);
    $result = mysqli_fetch_all($result, MYSQLI_ASSOC);

    // echo $main_loop_row['variant'];


    if (!empty($result)) {
// main loop starts
        $_GET['q'] = strtolower($main_loop_row['variant']);
        $basicWord = strtolower($main_loop_row['word']);
    ...
    <div class='header_wrapper'>";
        ...
        $big_html .= "<div class='logo_div'>
                    <div class='header_logo'>
                        <a href='" . $main_array['baseurl'] . "'><img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src='" . $url . "' onerror='this.onerror=null; this.src=\'" . $langLogo . "\' alt='" . strtoupper($main_array['logotext']) . "' height='55px' style='margin-bottom: -15px;' loading='lazy'</a>
                    </div>
...
// $big_html .= ($main_array['ismobile']) ? $main_array['alladcodes']['300'] : '';
// $big_html .= (!$main_array['ismobile']) ? $main_array['alladcodes']['body_top'] : '';
        ...
        if ($main_array['q'] and $main_array['lang']) {
            ...
            $sql = 'select word, ' . $main_array['lang'] . ', id, data from v3_word_frame where word=\'' . $basicWord . '\' limit 1';
            ...
            <?php foreach ($result as $row) {
                $sql1 = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM synonym AS a 
											WHERE a.`synsetid` = '" . $row['synsetid'] . "' LIMIT 1"));
                $synonym = $sql1['synonym'];
                switch ($row['pos']) {
                    case 'n':
                        $big_html .= '<span><label>' . $q . ' (Noun) : </label>';
                        break;
                    case 'v':
                        $big_html .= '<span><label>' . $q . ' (Verb) : </label>';
                        break;
                    case 'a':
                        $big_html .= '<span><label>' . $q . ' (Adjective) : </label>';
                        break;
                    case 'r':
                        $big_html .= '<span><label>' . $q . ' (Adverb) : </label>';
                        break;
                    case 's':
                        $big_html .= '<span><label>' . $q . ' (Adjective Satellite) : </label>';
                        break;
                ...
                if ($synonym) {
                    ...
                    if (($key = array_search($q, $synonym)) !== false) {
                        unset($synonym[$key]);
                    }
                    $big_html .= implode(',', $synonym);
                }

                $big_html .= ' (' . $row['definition'] . ')';
                if ($row['sample']) {
                    $big_html .= ' "' . $row['sample'] . '"';
                }
                $big_html .= '</span>';
            ...
            if ($main_array['lang'] == 'bengali' && $img_height) {
                ...
                $big_html .= "<span>
                                    ...
                                    <div class='dic_img'>
                                        <img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src=" . ($main_array['baurl'] . strtoupper($ba_img) . '.JPG' . '.webp') . " onerror='this.onerror=null; this.src='" . $main_array['baurl'] . strtoupper($ba_img) . '.JPG' . "' alt='" . $q . "' height='" . $img_height . 'px' . "' width='" . $img_width . 'px' . "'  title='" . $q . "' loading='lazy' />
                                    </div>
                                </span>";
                        ...
                        if ($key != 'result') {
                            ...
                            if ($key == 'm') {
                                ...
                                if (isset($value) && $value != null && sizeof($value) > 0) {
                                    ...
                                    $big_html .= '<span><label>Details : </label>';
            ...
            $big_html .= '<span><label>Other Refferences :</label>
                                    <button class="btn_default4 bdr_radius2 custom_bdr">
                                        <a href="http://the-definition.com/dictionary/' . stripslashes($q) . '" target="_blank">The Definition</a>
                                    </button>
                                    <button class="btn_default4 bdr_radius2 custom_bdr">
            ...
            if (isset($eng)) {
                ...
                $big_html .= "  <div class='accordion_collapse'>
            ...
            if ($q) {
                ...
                $big_html .= "<div class='custom_margin2'></div>
                                                    ...
                                                    <p>Meaning and definitions of " . stripslashes($q) . ", translation in " . str_replace('Bengali', 'Bangla', $main_array['ulang']) . "
                                                    ...
                                                    <p>What " . stripslashes($q) . " means in " . str_replace('Bengali', 'Bangla', $main_array['ulang']) . ", " . stripslashes($q) . " meaning
    ...
    <fieldset>';
        ...
        $big_html .= '<div class="fieldset_body inner_details"><div style="text-align: justify"><span> This is not just an ordinary English to ';
        ...
        $big_html .= str_replace("Bengali", "Bangla", $main_array['ulang']) . ' translate ' . str_replace("Bengali", "Bangla", $main_array['ulang']) . ' words to English, copy & paste any paragraph in the Reat Text box then tap on any word to get instant word meaning. This website also provides you English Grammar, TOEFL and most common words.';
        $big_html .= '</span>
            </div>
			<span>
			<button class="btn_default4 bdr_radius2 custom_bdr">
        ...
        $big_html .= '<fieldset><legend><h2>Topic Wise Words</h2></legend><div class="fieldset_body inner_details">';
        $rand = rand(1, 12700);
        ...
        if ($main_array['lang'] == 'bengali') {
            ...
            $big_html .= "<button class='btn_default2 bdr_radius2'>
                <a href='https://play.google.com/store/apps/details?id=com.bdword.e2bdictionary' target='_blank'>
                    <img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src='" . $contentUrl . "img/android-icon.webp' onerror='this.onerror=null; this.src='" . $contentUrl . "img/android-icon.png'' width='30px' height='30px' alt='icon' loading='lazy'><span>Android App</span>
                </a>
            </button>
            ...
            <button class='btn_default2 bdr_radius2'>
                <a href='https://itunes.apple.com/us/app/english-bengali-dictionary/id1213381239?ls=1&mt=8'
                    target='_blank'>
                    <img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src='" . $contentUrl . "img/ios-icon.webp' onerror='this.onerror=null; this.src='" . $contentUrl . "img/android-icon.png'' width='30px' height='30px' alt='icon' loading='lazy'><span>iPhone App</span>
                </a>
            </button>
            ...
            <button class='btn_default2 bdr_radius2'>
                <a href='https://chrome.google.com/webstore/detail/bdword-english-to-bengali/cogjmeckpkinhnidokapabgaoelhkbcl'
                    target='_blank'>
                    <img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src='" . $contentUrl . "img/chrome-icon.webp' onerror='this.onerror=null; this.src='" . $contentUrl . "img/android-icon.png'' width='30px' height='30px' alt='icon' loading='lazy'><span>Chrome Extension</span>
                </a>
            </button>";
            ...
            $big_html .= "<button class='btn_default2 bdr_radius2'>
                <a href='https://play.google.com/store/apps/details?id=com.bdword.e2" . $main_array['lang'] . "dictionary'
                    target='_blank'>
                    <img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src='" . $contentUrl . "img/android-icon.webp' onerror='this.onerror=null; this.src='" . $contentUrl . "img/android-icon.png'' width='30px' height='30px' alt='icon' loading='lazy'><span
            ...
            <button class='btn_default2 bdr_radius2'>
                <a href='" . $download_link . "' class='btn btn-primary btn-raised' target='_blank'>
                    <img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src='" . $contentUrl . "img/ios-icon.webp' onerror='this.onerror=null; this.src='" . $contentUrl . "img/ios-icon.png'' width='30px' height='30px' alt='icon' loading='lazy'><span
            ...
            <button class='btn_default2 bdr_radius2'>
                <a href='https://chrome.google.com/webstore/detail/english-to-any-language-d/apenapfkioiehfhgheabegngnfhnfbjh?hl=en&authuser=0'
                    target='_blank'>
                    <img src='data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=' data-src='" . $contentUrl . "img/chrome-icon.webp' onerror='this.onerror=null; this.src='" . $contentUrl . "img/chrome-icon.png'' width='30px' height='30px' alt='icon' loading='lazy'><span>Chrome Extension</span>
                </a>
            </button>";
    ...
    <script async type='text/javascript' src='" . $contentUrl . "js/responsivejs.js'></script>";
...
<script>
    ...
    window.onload = init;
        ...
        if ($is_test == true) {
            ...
            file_put_contents('/mnt/volume_sgp1_05/all_cache_files/afrikaans2/english-to-' . $_GET['lang'] . '-meaning-' . strtolower(str_replace(' ', '-', $_GET['q'])), $big_html);
        }
    } else {
		
        $domain = array_search($_GET['lang'], getLanguageArray());
        $html_output = "<!doctype html>
		<html>
		<head>
		<link rel='canonical' href='https://" . $domain . "/english-to-" . $_GET['lang'] . "-meaning-" . strtolower($main_loop_row['word']) . "' />
		</head>
		<body>

		<script>
		window.location.href='https://" . $domain . "/english-to-" . $_GET['lang'] . "-meaning-" . strtolower($main_loop_row['word']) . "';
		</script>

		</body>
		</html>";

        file_put_contents("/mnt/volume_sgp1_05/all_cache_files/afrikaans2/english-to-" . $_GET['lang'] . "-meaning-" . $main_loop_row['variant'], $html_output);
    }
}
