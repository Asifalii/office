...
$lang_array = array_flip($lang);

$domain = strtolower($lang_array[$langname]);
$lines = '';

$query = "SELECT DISTINCT(variant), word FROM `variants` WHERE word!=variant";
...
<html>
<head>
<link rel='canonical' href='https://".$domain."/english-to-".$langname."-meaning-".strtolower($topic['word'])."' />
</head>
<body>

<script>
window.location.href='https://".$domain."/english-to-".$langname."-meaning-".strtolower($topic['word'])."';
</script>

</body>
</html>";

file_put_contents("/mnt/volume_sgp1_05/all_cache_files/".$langname."/english-to-".$langname."-meaning-".$topic['variant'], $html_output);
