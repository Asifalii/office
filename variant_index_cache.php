...
// $big_html .= (!$main_array['ismobile']) ? $main_array['alladcodes']['body_top'] : '';
        ...
        if ($main_array['q'] and $main_array['lang']) {
            ...
            $big_html .= '<div><strong class="custom_font2 custom_margin3" style="color:black;">
			We are showing meaning for the Word "' . ucfirst($basicWord) . '". As "' . ucfirst($basicWord) . '" is the root word.</strong></div>';

            $a_bengali_master_query = "";
            if ($main_array['lang'] == 'bengali') {
                $a_bengali_master_query = "select details, mean, nex, prev, height, width from y_" . $main_array['lang'] . "_master where word='" . $basicWord . "' limit 1";
            } else {
                $a_bengali_master_query = "select details, mean, nex, prev from y_" . $main_array['lang'] . "_master where word='" . $basicWord . "' limit 1";
            }


            $a_bengali = mysqli_fetch_assoc(mysqli_query($conn, $a_bengali_master_query));
            $a_bengali_details = json_decode($a_bengali['details'], true);
            ...
            $ba_img = $basicWord;
            if ($main_array['lang'] == 'bengali') {
                $img_height = $a_bengali['height'];
                $img_width = $a_bengali['width'];
            }


            $mean_array = (isset($mean)) ? get_object_vars($mean) : array();

            $big_html .= '<span><div class="align_text  custom_font2">' . ucfirst(stripslashes($basicWord)) . ' : </div><div class="align_text2">' . $a_bengali['mean'] . '</div>
									<label class="img_align">Pronunciation: <button class="icon_button" onclick="textToSpeech(\'' . $basicWord . '\')"><img src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" height="35px" width="35px" data-src="' . $contentUrl . 'img/microphone1.png" title="Say The Word" loading="lazy"></button></label>
		                           <label class="img_align">Add to Favorite: <button class="icon_button" onclick="calFavs(\'' . $basicWord . '\', 1)"><img src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" height="35px" width="35px" data-src="' . $contentUrl . 'img/heart-add.png" title="Add To Favorites" loading="lazy"></button></label></span>';
            ...
            $variants_query = mysqli_query($conn, "select variant from variants where word='" . $basicWord . "'");
            while ($variants_row = mysqli_fetch_assoc($variants_query)) {
                $related_words[] = $variants_row['variant'];
            }
            ...
            $big_html .= '<span><label>Other Refferences :</label>
                                    <button class="btn_default4 bdr_radius2 custom_bdr">
                                        <a href="http://the-definition.com/dictionary/' . stripslashes($basicWord) . '" target="_blank">The Definition</a>
                                    </button>
                                    <button class="btn_default4 bdr_radius2 custom_bdr">
                                        <a href="http://dictionary.reference.com/browse/' . stripslashes($basicWord) . '?s=t" target="_blank">Dictionary.com</a>
                                    </button>
                                    <button class="btn_default4 bdr_radius2 custom_bdr">
                                        <a href="http://www.merriam-webster.com/dictionary/' . stripslashes($basicWord) . '" target="_blank">Merriam Webster</a>
                                    </button>
                                    <button class="btn_default4 bdr_radius2 custom_bdr">
                                        <a href="http://en.wikipedia.org/wiki/' . stripslashes($basicWord) . '" target="_blank">Wikipedia</a>
                                    </button>';
...
<script>
    ...
    window.onload = init;
        ...
        file_put_contents("/mnt/volume_sgp1_05/all_cache_files/" . $_GET['lang'] . "/english-to-" . $_GET['lang'] . "-meaning-" . $main_loop_row['variant'], $html_output);
    }
}
