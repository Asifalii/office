...
function getIndexPageTitle($q, $ulang)
{
    if($q!=''){
        $title = 'English to ' . str_replace("Bengali", "Bangla", $ulang) . ' Meaning of ' . strtoupper($q);
    }else{
        $title = '150k+ TRANSLATIONS | English to ' . str_replace("Bengali", "Bangla", $ulang) . ' Dictionary';
    }
...
function getTitle($q, $url, $sn, $lang, $ulang)
{
    ...
    switch ($sn) {
        ...
        case 'index':
            ...
            if (isset($q) && $q!='') {
                if (preg_match("/^[a-z]$/", $q)) {
                    $title = 'English to ' . str_replace("Bengali", "Bangla", $ulang) . ' Meaning of ' . stripslashes($q);
                } else {
                    //$title = str_replace("Bengali", "Bangla", $ulang) . ' to English Meaning of ' . stripslashes($q) . ' - ' . $url;
                    $title = 'English to '.str_replace("Bengali", "Bangla", $ulang) .' Meaning/Translation of ' .ucfirst(stripslashes($q));
                }
            }else{
                $title = 'English to '.str_replace("Bengali", "Bangla", $ulang).' Dictionary/Translation';
            }
            break;
...
function getScriptName()
{
    ...
    // $document_uri = (!empty($_SERVER['DOCUMENT_URI']))?strtolower(str_replace('.php', '', trim($_SERVER['DOCUMENT_URI'], '/'))) :'';
